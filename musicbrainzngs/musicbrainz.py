...
from musicbrainzngs import compat, mbxml, util
...
VALID_INCLUDES = {
	...
	+ TAG_INCLUDES
	+ RELATION_INCLUDES,
	"release-group": ["artists", "releases", "discids", "media", "artist-credits", "annotation", "aliases"]
	+ TAG_INCLUDES
	+ RATING_INCLUDES
	+ RELATION_INCLUDES,
...
VALID_BROWSE_INCLUDES = {
	...
	"release": ["artist-credits", "labels", "recordings", "isrcs", "release-groups", "media", "discids"]
	+ RELATION_INCLUDES,
...
VALID_SEARCH_FIELDS = {
	...
	"instrument": ["alias", "comment", "description", "iid", "instrument", "instrumentaccent", "tag", "type"],
	"label": [
		"alias",
		"area",
		"begin",
	...
	"series": ["alias", "comment", "orderingattribute", "series", "seriesaccent", "sid", "tag", "type"],
	"work": [
		"alias",
		"arid",
		"artist",
...
def _check_filter_and_make_params(entity, includes, release_status=[], release_type=[]) -> dict:
	...
	if (
		...
		raise InvalidFilterError("Can't have a release type with no releases or release-groups involved")
...
class _rate_limit:
	...
	def _update_remaining(self):
		...
		if self.remaining_requests is None:
			...
			self.remaining_requests = min(self.remaining_requests, float(limit_requests))
		...
		with self.lock:
			if do_rate_limit:
				...
				while self.remaining_requests < 0.999:
					time.sleep((1.0 - self.remaining_requests) * (limit_requests / limit_interval))
					self._update_remaining()
...
def _mb_request(path, method="GET", auth_required=AUTH_NO, client_required=False, args=None, data=None, body=None):
	...
	# query string.
	url = urlunparse(("https" if https else "http", hostname, "/ws/2/%s" % path, "", urlencode(newargs), ""))
...
def get_instrument_by_id(id, includes=[], release_status=[], release_type=[]):
	...
	*Available includes*: {includes}
	"""
	params = _check_filter_and_make_params("instrument", includes, release_status, release_type)
...
def get_event_by_id(id, includes=[], release_status=[], release_type=[]):
	...
	*Available includes*: {includes}
	"""
	params = _check_filter_and_make_params("event", includes, release_status, release_type)
...
def get_releases_by_discid(id, includes=[], toc=None, cdstubs=True, media_format=None):
	...
	*Available includes*: {includes}
	"""
	params = _check_filter_and_make_params("discid", includes, release_status=[], release_type=[])
	if toc:
		params["toc"] = toc
	if not cdstubs:
		params["cdstubs"] = "no"
...
def browse_artists(recording=None, release=None, release_group=None, work=None, includes=[], limit=None, offset=None):
	"""Get all artists linked to a recording, a release or a release group.
	You need to give one MusicBrainz ID.

	*Available includes*: {includes}
	"""
	params = {"recording": recording, "release": release, "release-group": release_group, "work": work}
...
def browse_releases(
	...
	*Available includes*: {includes}
	...
	return _browse_impl("release", includes, limit, offset, params, release_status, release_type)
...
def browse_release_groups(artist=None, release=None, release_type=[], includes=[], limit=None, offset=None):
	...
	*Available includes*: {includes}
	"""
	params = {"artist": artist, "release": release}
	return _browse_impl("release-group", includes, limit, offset, params, [], release_type)
