...
def show_collections():
	...
	for collection in result["collection-list"]:
		...
		if "entity-type" in collection:
			print(
				'"{name}" by {editor} ({cat}, {count} {entity}s)\n\t{mbid}'.format(
...
if __name__ == "__main__":
	...
	parser.add_option("-a", "--add", metavar="RELEASE-ID", help="add a release to the collection")
	parser.add_option("-r", "--remove", metavar="RELEASE-ID", help="remove a release from the collection")
	parser.add_option(
		"-t", "--type", metavar="TYPE", default="release", help="type of the collection (default: release)"
	)
	...
	if args:
		...
		if options.add:
			if option.type == "release":
				musicbrainzngs.add_releases_to_collection(collection_id, [options.add])
			else:
				sys.exit("only release collections can be modified ATM")
		elif options.remove:
			if option.type == "release":
				musicbrainzngs.remove_releases_from_collection(collection_id, [options.remove])
			else:
				sys.exit("only release collections can be modified ATM")
		else:
